...
echo "### Formula tests"
mcs -langversion:7.2 -out:"$OUT/FormulaTests.exe" Tests/FormulaTests.cs Source/LoadSupport/Core/LoadSupportFormula.cs
mono "$OUT/FormulaTests.exe"

echo; echo "### Integration tests"
: "${RIMWORLD_MANAGED:?set RIMWORLD_MANAGED}"; : "${HARMONY_DLL:?set HARMONY_DLL}"
cp "$RIMWORLD_MANAGED"/*.dll "$OUT/"; cp "$HARMONY_DLL" "$OUT/"; cp 1.6/Assemblies/LoadSupport.dll "$OUT/"
mcs -langversion:7.2 -nostdlib -noconfig -out:"$OUT/IntegrationTests.exe" \
  -r:"$API/mscorlib.dll" -r:"$API/System.dll" -r:"$API/System.Core.dll" -r:"$API/Facades/netstandard.dll" \
  -r:"$OUT/Assembly-CSharp.dll" -r:"$OUT/UnityEngine.CoreModule.dll" -r:"$OUT/0Harmony.dll" -r:"$OUT/LoadSupport.dll" \
